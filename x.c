...
#if LIGATURES_PATCH
void xresetfontsettings(uint32_t mode, Font **font, int *frcflags) {
    *font = &dc.font;
...
void cmessage(XEvent *e) {
    ...
    if (e->xclient.message_type == xw.xembed && e->xclient.format == 32) {
        ...
        if (xw.XdndSourceVersion > 5)
            return;
        xdndenter(e);
    } else if (e->xclient.message_type == xw.XdndPosition && xw.XdndSourceVersion <= 5) {
        xdndpos(e);
    } else if (e->xclient.message_type == xw.XdndDrop && xw.XdndSourceVersion <= 5) {
        xdnddrop(e);
#endif // DRAG_AND_DROP_PATCH
    }
}
